...
public class GetAllNotes implements CommandAbstraction {
	
	private EnhancedObjectService objectService;
	private ObjectCrud objectCrud;
	private ObjectConverter objectConverter;
	private final Log logger = LogFactory.getLog(GetAllNotes.class);

	public GetAllNotes(EnhancedObjectService objectService, ObjectCrud objectCrud, ObjectConverter objectConverter) {
	...
	public List<Object> invokeCommand(MiniAppCommandBoundary boundary) {
		...
		.addAll(
		objectCrud
		.findAllByCreatedByAndTypeAndAlias(createdBy,type,alias,PageRequest.of(0, 10, Direction.DESC, "creationTimestamp", "objectId"))
		.stream()
